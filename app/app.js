  ...
  $('.' + classname).click(function() {
    ...
    map.addLayer({
      ...
      map.addLayer({
        ...
        "source": {
            "type": "vector",
            "tiles": ["https://geospatial.jrc.ec.europa.eu/geoserver/gwc/service/wmts?layer=dopa_explorer_3:global_dashboard&tilematrixset=EPSG:900913&Service=WMTS&Request=GetTile&Version=1.0.0&Format=application/x-protobuf;type=mapbox-vector&TileMatrix=EPSG:900913:{z}&TileCol={x}&TileRow={y}"]
            },
        "source-layer": "global_dashboard",
    ...
    map.addLayer({
      "id": "ecoregion_master_202104",
      "type": "fill",
      "source": {
          "type": "vector",
          "tiles": ["https://geospatial.jrc.ec.europa.eu/geoserver/gwc/service/wmts?layer=dopa_explorer_3:ecoregion_master_202104&tilematrixset=EPSG:900913&Service=WMTS&Request=GetTile&Version=1.0.0&Format=application/x-protobuf;type=mapbox-vector&TileMatrix=EPSG:900913:{z}&TileCol={x}&TileRow={y}"]
          },
      "source-layer": "ecoregion_master_202104",
...
layer_country.addEventListener('change', function() {
  ...
  if (layer_country_value =='prot_mar_perc_ind'){
    map.setFilter("global_dashboard", [">", "prot_mar_perc_ind", 0]);
    map.setPaintProperty('global_dashboard', 'fill-color', 
  ...
  map.setFilter("global_dashboard", [">", "prot_perc_ind",-1]);
    map.setPaintProperty('global_dashboard', 'fill-color', 
    ['interpolate',['linear'],['get', layer_country_value],
...
$( ".live_select" ).click(function() {
  ...
  map.setFilter("global_dashboard", ["in", "id", "xxx"]);
...
// Country Popup
        map.on('click', 'global_dashboard', function (e) {

          map.setFilter("dopa_geoserver_wdpa_master_202101_o1", ["in", "iso3", e.features[0].properties.iso3_digit]);

          map.setFilter("global_dashboard", ["!in", "iso2_digit", e.features[0].properties.iso2_digit]);
        ...
        rangeSliderwdpa();
          ...
          $('#pa_stats').show();
             ...
             var country_stats_rest = "https://geospatial.jrc.ec.europa.eu/geoserver/wfs?request=getfeature&version=1.0.0&service=wfs&typename=dopa_explorer_3:global_dashboard&propertyname=name_c,iso3_digit,iso2_digit,&SORTBY=iso2_digit&CQL_FILTER=iso2_digit='"+e.features[0].properties.iso2_digit+"'&outputFormat=application%2Fjson";
          ...
          map.on('mouseenter', 'global_dashboard', function () {
          map.getCanvas().style.cursor = 'pointer';
          });
           
          map.on('mouseleave', 'global_dashboard', function () {
          map.getCanvas().style.cursor = '';
          map.setFilter("country_high", ["in", "iso2_digit", "xxx"]);
          });

          map.on("moveend", function () {
            var features = map.queryRenderedFeatures({ layers: ["global_dashboard"] });
            if (features) {
            ...
            map.on("mousemove", "global_dashboard", function (e) {
            map.getCanvas().style.cursor = "pointer";
            map.setFilter("country_high", ["in", "iso2_digit", e.features[0].properties.iso2_digit]);
            var prot_mar_perc_ind = e.features[0].properties.prot_mar_perc_ind;
            if (prot_mar_perc_ind == null){
...
// Ecoregion Popup
          map.on('click', 'ecoregion_master_202104', function (e) {
            ...
            var marine = e.features[0].properties.is_marine;
              ...
              var pa_bb_url = "https://geospatial.jrc.ec.europa.eu/geoserver/wfs?request=getfeature&version=1.0.0&service=wfs&typename=dopa_explorer_3:ecoregion_master_202104&propertyname=id,&SORTBY=id&CQL_FILTER=id='"+e.features[0].properties.id+"'&outputFormat=application%2Fjson";
            ...
            map.on('mouseenter', 'ecoregion_master_202104', function () {
            map.getCanvas().style.cursor = 'pointer';
            });
             
            map.on('mouseleave', 'ecoregion_master_202104', function () {
            map.getCanvas().style.cursor = '';
            map.setFilter("ecoregion_high", ["in", "id", "xxx"]);
            });
            ...
            map.on("moveend", function () {
              ...
              var features = map.queryRenderedFeatures({ layers: ["ecoregion_master_202104"] });
              if (features) {
              ...
              map.on("mousemove", "ecoregion_master_202104", function (e) {
                map.setFilter("ecoregion_high", ["in", "id", e.features[0].properties.id]);
                var marine = e.features[0].properties.is_marine;
                if (marine == 'yes') {
